...
import { getUserPayments } from '@/lib/dpw-client';
...
export async function GET(
  ...
  // Youth can only view their own payments
  if (token.role === 'youth' && token.userId !== id) {
    return NextResponse.json({ success: false, error: { code: 'FORBIDDEN', message: 'Cannot view other users\' payments' } }, { status: 403 });
  }
