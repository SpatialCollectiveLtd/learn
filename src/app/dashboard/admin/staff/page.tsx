...
import { UserPlus, Trash2, ArrowLeft, CheckCircle, AlertCircle } from "lucide-react";
...
export default function AdminStaffManagement() {
  ...
  const loadTrainers = async () => {
    try {
      const token = localStorage.getItem('staffToken');
      const response = await axios.get(`${API_URL}/api/staff?role=trainer`, {
  ...
  const handleAddTrainer = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.success) {
        setMessage({ type: 'success', text: 'Trainer added successfully!' });
        setFormData({ staffId: '', fullName: '', email: '' });
      ...
      setMessage({ 
        type: 'error', 
        text: error.response?.data?.message || 'Failed to add trainer' 
      });
    }
  ...
  const handleDeleteTrainer = async (staffId: string) => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: 'Trainer removed successfully' });
      loadTrainers();
    } catch (error: any) {
      setMessage({ 
        type: 'error', 
        text: error.response?.data?.message || 'Failed to remove trainer' 
      });
    }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    required
                  />
                  <p className="mt-1 text-xs text-[#737373]">Must end with 'T' for Trainer</p>
                </div>
        ...
        <div className="bg-[#1F2121] border border-[#2a2a2a] rounded-lg overflow-hidden">
          ...
          {staffMembers.length === 0 ? (
            <div className="p-12 text-center">
              <UserPlus className="w-12 h-12 text-[#737373] mx-auto mb-4" />
              <p className="text-[#a3a3a3] mb-2">No trainers yet</p>
              <p className="text-sm text-[#737373]">Click "Add New Trainer" to get started</p>
            </div>
          ) : (
                  ...
                  {staffMembers.map((staff) => (
                    <tr key={staff.staff_id} className="hover:bg-[#2a2a2a]/50">
                      ...
                      <td className="px-6 py-4 text-right">
                        <button
                          onClick={() => handleDeleteTrainer(staff.staff_id)}
