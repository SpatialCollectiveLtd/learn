...
// The 9 youth with newly assigned IDs
const missing9Youth = [
  {
    uniqueId: "KAR286EM",
    firstName: "Emily",
...
// Add the 9 missing youth
missing9Youth.forEach(youth => {
  allYouth.push({
    ...youth,
...
console.log(`Total youth (291 + 9 missing): ${allYouth.length}`);
...
if (total !== 300) {
  ...
  console.log(`\n✅ Total matches 300 youth exactly!`);
}
...
// Create master CSV with all fields
const allAllocated = [
  ...digitizationYouth,
  ...dropoutYouth,
  ...microtaskingYouth,
...
console.log(`\n=== 9 NEWLY ADDED YOUTH ===`);
missing9Youth.forEach(y => {
  const allocated = allAllocated.find(a => a.uniqueId === y.uniqueId);
  console.log(`${y.uniqueId} - ${y.firstName} ${y.lastName} → ${allocated ? allocated.module : 'NOT FOUND'}`);
});
