...
// The actual raw data you provided
const rawData = `UniqueID	FirstName	LastName	NationalID	PhoneNumber_mpesa	Age	Gender	Settlement	Zone	Disability
KAY2070EM	Emily	Munywoki	41435294	746139777	22	Female	kayole_soweto	muungano	
...
let blankDisability = 0;
let noneDisability = 0;
let pregnantCount = 0;
...
lines.forEach(line => {
  ...
  if (disability === '') {
    blankDisability++;
    withoutDisability++;
  } else if (disability.toLowerCase() === 'none') {
    noneDisability++;
    withoutDisability++;
  } else {
    withDisability++;
    disabilityDetails.push({
      uniqueId,
      name: `${firstName} ${lastName}`,
      disability
    });
    
    if (disability.toLowerCase().includes('pregnant')) {
      pregnantCount++;
    }
  }
});

console.log('=== ACTUAL DISABILITY COUNT FROM YOUR DATA ===\n');
console.log(`Total Youth: ${totalYouth}`);
console.log(`\nWith Disabilities: ${withDisability}`);
console.log(`Without Disabilities (blank or "None"): ${withoutDisability}`);
console.log(`  - Blank disability field: ${blankDisability}`);
console.log(`  - Explicitly marked "None": ${noneDisability}`);
console.log(`\nPregnant (included in disabilities): ${pregnantCount}`);

console.log(`\n=== BREAKDOWN OF ${withDisability} YOUTH WITH DISABILITIES ===\n`);
...
disabilityDetails.forEach(person => {
  const d = person.disability.toLowerCase();
  const hasMultiple = (d.match(/;/g) || []).length > 0;
  
  if (hasMultiple && !d.includes('pregnant')) {
    disabilityGroups.multiple.push(person);
  } else {
    if (d.includes('vision')) disabilityGroups.vision.push(person);
    if (d.includes('hearing')) disabilityGroups.hearing.push(person);
    if (d.includes('mobility')) disabilityGroups.mobility.push(person);
    if (d.includes('cognitive')) disabilityGroups.cognitive.push(person);
    if (d.includes('self-care')) disabilityGroups.selfCare.push(person);
    if (d.includes('communication')) disabilityGroups.communication.push(person);
    if (d.includes('pregnant')) disabilityGroups.pregnant.push(person);
  }
});

console.log(`Vision difficulties: ${disabilityGroups.vision.length}`);
console.log(`Hearing difficulties: ${disabilityGroups.hearing.length}`);
console.log(`Mobility difficulties: ${disabilityGroups.mobility.length}`);
console.log(`Cognitive difficulties: ${disabilityGroups.cognitive.length}`);
console.log(`Self-care difficulties: ${disabilityGroups.selfCare.length}`);
console.log(`Communication difficulties: ${disabilityGroups.communication.length}`);
console.log(`Pregnant (listed as disability): ${disabilityGroups.pregnant.length}`);
console.log(`Multiple disabilities: ${disabilityGroups.multiple.length}`);

// Save detailed list
fs.writeFileSync('actual-disability-breakdown.json', JSON.stringify({
  summary: {
    total: totalYouth,
    withDisabilities: withDisability,
    withoutDisabilities: withoutDisability
  },
  details: disabilityDetails,
  groups: disabilityGroups
}, null, 2));

console.log(`\n✅ Detailed breakdown saved to: actual-disability-breakdown.json`);

console.log(`\n=== MY PREVIOUS ANALYSIS WAS WRONG ===`);
console.log(`I previously said: 216 with disabilities, 28 without`);
console.log(`Actual count: ${withDisability} with disabilities, ${withoutDisability} without`);
console.log(`\nThis is a MAJOR difference and affects module allocation!`);
